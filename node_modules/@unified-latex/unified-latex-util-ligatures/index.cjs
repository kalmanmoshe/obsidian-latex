...
function hashNodes(nodes) {
  return nodes.map((node) => unifiedLatexUtilMatch.match.macro(node) ? `\\${node.content}` : node.content).join(" ");
}
function isMacroOrStringArray(nodes) {
  return nodes.some((node) => unifiedLatexUtilMatch.match.macro(node) || unifiedLatexUtilMatch.match.string(node));
}
function ligatureToUnicode(nodes) {
  if (!isMacroOrStringArray(nodes)) {
    return null;
  }
  if (nodes.length === 1 && unifiedLatexUtilMatch.match.macro(nodes[0], " ") && nodes[0].escapeToken == null) {
    return makeString(" ");
  }
...
function createMatchers() {
  return {
    isMacro: unifiedLatexUtilMatch.match.anyMacro,
    isWhitespace: unifiedLatexUtilMatch.match.whitespace,
    isRecognized: (nodes, whitespaceAllowed = false) => {
      const nodesToTest = [...nodes];
      if (nodes.length === 2 && unifiedLatexUtilMatch.match.macro(nodes[0])) {
        const arg = nodes[1];
        if (unifiedLatexUtilMatch.match.group(arg) && arg.content.length === 1) {
    ...
    isSplitable: (node) => unifiedLatexUtilMatch.match.anyString(node) && node.content.length > 1,
    split: (node) => [
...
function parseLigatures(ast) {
  ...
  return unifiedLatexUtilPegjs.LigaturesPegParser.parse(ast, createMatchers());
}
