...
const unifiedLatexUtilPackages = require("@unified-latex/unified-latex-util-packages");
...
${Object.entries(OBSOLETE).map(([name, suggestion]) => `  * \`${name}\`: ${suggestion}`).join("\n")}
...
`;
const unifiedLatexLintObsoletePackages = index.lintRule({ origin: "unified-latex-lint:obsolete-packages" }, (tree, file, options) => {
  for (const packageStr of unifiedLatexUtilPackages.listPackages(tree)) {
...
exports.unifiedLatexLintObsoletePackages = unifiedLatexLintObsoletePackages;
