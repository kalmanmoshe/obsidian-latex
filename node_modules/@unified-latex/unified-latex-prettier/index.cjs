...
const tikz = require("@unified-latex/unified-latex-ctan/package/tikz");
const unifiedLatexUtilPgfkeys = require("@unified-latex/unified-latex-util-pgfkeys");
...
function formatEnvSurround(node) {
  const env = unifiedLatexUtilPrintRaw.printRaw(node.env);
  return {
    envName: env,
...
function formatDocArray(nodes, docArray, options) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    switch (rawNode.type) {
      case "comment":
        if (!rawNode.sameline && previousNode && !unifiedLatexUtilMatch.match.comment(previousNode) && !unifiedLatexUtilMatch.match.parbreak(previousNode)) {
          ret.push(hardline);
        }
      ...
      case "macro":
        if (renderInfo.breakBefore || renderInfo.breakAround) {
          if (previousNode) {
            if (ret[ret.length - 1] === line || ret[ret.length - 1] === hardline) {
              ret.pop();
              ret.push(hardline);
            } else if (!unifiedLatexUtilMatch.match.comment(previousNode) && !unifiedLatexUtilMatch.match.parbreak(previousNode)) {
              ret.push(hardline);
            }
          }
        }
        ...
        if (renderInfo.breakAfter || renderInfo.breakAround) {
          if (nextNode) {
            if (unifiedLatexUtilMatch.match.whitespace(nextNode)) {
              ret.push(hardline);
              i++;
            } else if (unifiedLatexUtilMatch.match.parbreak(nextNode)) ;
            else if (!unifiedLatexUtilMatch.match.comment(nextNode)) {
              ret.push(hardline);
            }
          }
        }
...
function printArgumentPgfkeys(nodes, options) {
  ...
  const parsed = unifiedLatexUtilPgfkeys.parsePgfkeys(nodes, { allowParenGroups });
  const content = [];
  for (const part of parsed) {
    const isLastItem = part === parsed[parsed.length - 1];
    if (part.itemParts) {
      const parts = part.itemParts.map(
        (node) => unifiedLatexUtilPrintRaw.printRaw(node, { asArray: true }).map(
          (token) => token === unifiedLatexUtilPrintRaw.linebreak ? hardline : token
        )
      );
...
function printTikzArgument(path, print, options) {
  ...
  try {
    const tikzAst = tikz.parse(nodes);
...
function printTikzPathSpecNode(node, path, print) {
  switch (node.type) {
    ...
    case "coordinate":
      return [unifiedLatexUtilPrintRaw.printRaw(node.prefix), "(", unifiedLatexUtilPrintRaw.printRaw(node.content), ")"];
    case "line_to":
      return node.command;
    ...
    case "svg_operation": {
      ...
      const rest = node.options ? [group(indent([line, unifiedLatexUtilPrintRaw.printRaw(node.content)]))] : [" ", unifiedLatexUtilPrintRaw.printRaw(node.content)];
    ...
    case "foreach": {
      ...
      const doc2 = [
        ...comments,
        unifiedLatexUtilPrintRaw.printRaw(node.start),
        " ",
        unifiedLatexUtilPrintRaw.printRaw(variables)
      ];
      if (node.options) {
        doc2.push(" ", indent(printOptionalArgs(node.options)));
      }
      doc2.push(" ", "in", " ", group(indent(list)));
      ...
      switch (commandType) {
        ...
        case "group": {
          ...
          unifiedLatexUtilTrim.trim(groupContent);
          doc2.push(
            " ",
            indent(
              group([
...
class TikzArgumentPrinter {
  ...
  toDoc() {
    ...
    if (startArg.optionalArg) {
      ...
      doc2.push(printOptionalArgs(startArg.optionalArg.content));
    }
    const innerDoc = [];
    doc2.push(group([indent(innerDoc), ";"]));
    let cycle = -1;
  ...
  eatNode() {
    ...
    const innerDoc = [];
    const commentBlock = [];
    const doc2 = [commentBlock, "node", group(indent(innerDoc))];
      ...
      switch (node.type) {
        ...
        case "operation": {
          if (node.content.content === "at") {
            ...
            if (!nextNode || !(nextNode.type === "coordinate" || nextNode.type === "unknown" && unifiedLatexUtilMatch.match.anyMacro(nextNode.content))) {
              shouldBail = true;
              continue;
            }
        ...
        case "unknown": {
          if (unifiedLatexUtilMatch.match.group(node.content)) {
  ...
  eatOperation() {
    ...
    if (options.optionalArg) {
      doc2.push(indent(printOptionalArgs(options.optionalArg.content)));
    }
    return doc2;
  }
}
...
function printArgument(path, print, options) {
  ...
  if (parentRenderInfo.pgfkeysArgs) {
    const leadingComment = node.content.length > 0 && unifiedLatexUtilMatch.match.comment(node.content[0]) && node.content[0].sameline ? node.content[0] : null;
    const content2 = leadingComment ? node.content.slice(1) : node.content;
    unifiedLatexUtilTrim.trim(content2);
  ...
  if (unifiedLatexUtilMatch.match.comment(node.content[node.content.length - 1])) {
...
function hasPreambleCode(nodes) {
  return nodes.some((node) => unifiedLatexUtilMatch.match.macro(node, "documentclass"));
}
...
function printComment(path, _print2, _options) {
  ...
  const content = [leadingWhitespace, "%" + unifiedLatexUtilPrintRaw.printRaw(node.content)];
  return content;
}
...
function printAlignedEnvironment(path, print, options) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    content.push(row);
    if (rowSep) {
      content.push(unifiedLatexUtilPrintRaw.printRaw(rowSep));
    }
    if (rowSep && trailingComment) {
      content.push(" ");
    }
    if (trailingComment) {
      content.push(["%", unifiedLatexUtilPrintRaw.printRaw(trailingComment.content)]);
    }
    if (rowSep || trailingComment) {
      content.push(hardline);
    }
  ...
  if (leadingComment) {
    content.unshift(
      ...
      "%" + unifiedLatexUtilPrintRaw.printRaw(leadingComment.content),
      hardline
    );
...
function formatAlignedContent(nodes) {
  ...
  const rows = unifiedLatexUtilAlign.parseAlignEnvironment(nodes);
  const numCols = Math.max(...rows.map((r) => r.cells.length));
  const rowSeps = rows.map(({ rowSep }) => unifiedLatexUtilPrintRaw.printRaw(rowSep || []));
  ...
  const renderedRows = rows.map(({ cells, colSeps }) => ({
    cells: cells.map((nodes2) => {
      unifiedLatexUtilTrim.trim(nodes2);
      return unifiedLatexUtilPrintRaw.printRaw(nodes2);
    }),
    seps: colSeps.map((nodes2) => unifiedLatexUtilPrintRaw.printRaw(nodes2))
  }));
  const colWidths = [];
...
function printLatexAst(path, options, print) {
  ...
  switch (node.type) {
    ...
    case "group":
      const content = unifiedLatexUtilPrintRaw.printRaw(node.content, {
        asArray: true
      }).map((token) => token === unifiedLatexUtilPrintRaw.linebreak ? hardline : token);
      return ["{", ...content, "}"];
    case "inlinemath":
...
const parsers = {
  "latex-parser": {
    parse: unifiedLatexUtilParse.parse,
    astFormat: "latex-ast",
