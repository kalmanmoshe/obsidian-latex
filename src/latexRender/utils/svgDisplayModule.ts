...
export class svgDisplayModule extends Modal {
	...
	constructor(cacheFolderPath: string, cache: Map<string, Set<string>>) {
		super(app);
		...
		for (const [hash, fileSet] of this.cache.entries()) {
			...
			if (fs.existsSync(svgPath)) {
				try {
					...
					const svgEl = entryContainer.createDiv({
						cls: 'svg-display',
					});
					svgEl.innerHTML = svg;
				} catch (err) {
					entryContainer.createEl('p', {
						text: 'Error reading SVG file.',
					});
				}
			} else {
				// Inform the user that the SVG file is not found in the cache folder
				entryContainer.createEl('p', {
					text: 'SVG file not found in cache.',
				});
			}
		}
	}
