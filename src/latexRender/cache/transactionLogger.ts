...
export function createTransactionLogger(maxEntries: number = 10): TransactionLogger {
	...
	return {
        ...
        getLatestChange: () => history.length > 0 ? transactionHistory[transactionHistory.length - 1] : null,
        hasRecentChanges: (expireMs =1000) => {
			const now = Date.now();
			return transactionHistory.some(change => now - change.timestamp < expireMs);
		},
