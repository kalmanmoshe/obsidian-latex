...
export default class  CompiledFileCache {
    ...
    private virtualCache?: VirtualCache//Map<string, string>;
    ...
    private async removeUntraceableFiles() {
        ...
        const filesToRemove = cacheFolderFiles.filter(file => !cacheHashes.includes(file.split(".")[0]));
    ...
    private async removeHashsWithNoCorrespondingPDF() {
        const cacheFolderHashes = this.cache.listCacheFiles().map(file => file.split(".")[0]);
        const cacheHashes = [...this.cacheMap.keys()];
        const hashesToRemove = cacheHashes.filter(hash => !cacheFolderHashes.includes(hash));
    ...
    public removeAllCached(): void {
        const hashes = this.cache.listCacheFiles().map(file => file.split(".")[0]);
...
class VirtualCache implements FileCache {
    ...
    listCacheFiles() {
        return [...(this.cache.keys() || [])].map(hash => `${hash}.${cacheFileFormat}`);
    }
}
class PhysicalCache implements FileCache {
    ...
    listCacheFiles() {
        return fs.existsSync(this.getCacheFolderPath())
            ? fs.readdirSync(this.getCacheFolderPath()).filter(f => f.endsWith(`.${cacheFileFormat}`))
            : [];
    }
