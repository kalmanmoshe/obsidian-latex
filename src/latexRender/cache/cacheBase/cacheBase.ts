...
export abstract class CacheBase {
	...
	isValidFileExtension(extension: any) {
		...
		return (
			this.cacheFileExtensions === undefined ||
			this.cacheFileExtensions.includes(extension)
		);
	}
	...
	ensureIsValidFileExtension(extension: string): string {
		if (!this.isValidFileExtension(extension)) {
			...
			throw new Error(
				`Invalid file extension: ${extension}. Valid extensions are: ${this.cacheFileExtensions!.join(', ')}`,
			);
		}
		return extension;
	}
