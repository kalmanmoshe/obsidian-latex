...
interface OpsTable {
    dir: {
        node: {
            ...
            unlink: (parent: { contents: { [x: string]: any; }; timestamp: number; }, name: string | number) => void;
            rmdir: (parent: { contents: { [x: string]: any; }; timestamp: number; }, name: string) => void;
...
export class MEMFS {
        ...
        this.ops_table = {
            dir: {
                node: {
                    ...
                    rename: (old_node, new_dir, new_name) => this.rename(old_node, new_dir, new_name),
            ...
            file: {
                ...
                stream: {
                    ...
                    write: (stream, buffer, offset, length, position, canOwn) => this.write(stream, buffer, offset, length, position, canOwn),
                    allocate: (stream, offset, length) => this.allocate(stream, offset, length),
                    mmap: (stream, length, position, prot, flags) => this.mmap(stream, length, position, prot, flags),
                    msync: (stream, buffer, offset, length, mmapFlags) => this.msync(stream, buffer, offset, length, mmapFlags),
                },
            },
            link: {
                node: {
    ...
    read(stream: FSStream, buffer: any, offset: number, length: number, position: number) {
    ...
    write(stream: FSStream, buffer: any, offset: number, length: number, position: number, canOwn: boolean) {
    ...
    msync(stream: FSStream, buffer: any, offset: number, length: number, mmapFlags: number) {
