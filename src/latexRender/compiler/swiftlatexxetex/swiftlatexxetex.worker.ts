...
function transferTexFileToHost(filename) {
	try {
		let content = FS.readFile(
			self.constants.TEXCACHEROOT + '/' + filename,
			{
				encoding: 'binary',
			},
		);
...
var wasmBinaryFile = wasmBinaryFileImport;
...
var FS = {
	...
	lookupPath(
		path: String,
		opts: { follow_mount?: boolean; recurse_count?: number } = {},
	) {
	...
	open(path, flags, mode) {
		...
		if (flags & 64) {
			if (node) {
				...
				if (flags & 128) {
					console.log(
						`FS.open: File "${path}" exists, but flags indicate it should not.`,
					);
...
var ccall = (
	...
	argTypes: Array<any> | undefined,
	args: (Arguments | Array<any>) | undefined,
...
var wasmImports = {
	...
	/** @export */
	kpse_find_file_js: _kpse_find_file_js,
	/** @export */
	strftime: _strftime,
	/** @export */
...
var _compileLaTeX = (Module['_compileLaTeX'] =
	createExportWrapper('compileLaTeX'));
var _compileFormat = (Module['_compileFormat'] =
	createExportWrapper('compileFormat'));
var _compileBibtex = (Module['_compileBibtex'] =
	createExportWrapper('compileBibtex'));
var _setMainEntry = (Module['_setMainEntry'] =
	createExportWrapper('setMainEntry'));
...
var dynCall_iijiji = (Module['dynCall_iijiji'] =
	createExportWrapper('dynCall_iijiji'));
