 ...
 */
export function getSectionsFromMatching(
	sections: SectionCache[],
	fileText: string,
	codeBlock: string,
): MarkdownSectionInformation[] | undefined {
...
function extractSectionCheck(indexes: number[]) {
	...
	if (indexes.length > 1) {
		throw new Error(
			'LatexRender: Multiple sections found with the same source. Please ensure unique section headers.',
		);
	}
	const index = indexes[0];
	if (index <= 0) {
		throw new Error(
			'LatexRender: Invalid section index found. This should not happen.',
		);
	}
	return index;
}
