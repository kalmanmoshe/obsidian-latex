import { Argument, Ast, Macro, Path, Node, ContentNode } from './typs/astNodes';
...
export function claenUpPaths(ast: Ast) {
	const condition = (node: Node) =>
		node instanceof Macro && !!node.content.match(pathMatchRegex);
	function action(ast: Node, index: number) {
		if (!contentInNodeAndArray(ast)) return;
		const matchIndex = ast.content.findIndex(
			(node, i) => i > index && node.isString() && node.content === ';',
		);
		if (matchIndex === -1) {
			...
			if (
				el.args &&
				el.args.length > 0 &&
				el.args[el.args.length - 1].closeMark === ';'
			) {
				if (el.args.length !== 1)
					throw new Error('Path with args but no semicolon found');
				ast.content[index] = new Path(
					(ast.content[index] as Macro).content,
					el.args[0].content,
				);
				return;
			} else {
				throw new Error(
					'at claen up paths action: found path but no semiolon giving up in this path',
				);
			}
		}
		if (!ast.isContentNode() || !Array.isArray(ast.content)) {
		...
		ast.content.splice(
			index,
			lengthBetweenIndexes(index, matchIndex),
			path,
		);
	}
...
function contentInNodeAndArray<T extends Node>(
	node: T,
): node is T & { content: Node[] } {
