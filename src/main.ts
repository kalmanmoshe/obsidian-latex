...
class VecProcessor {
  ...
  constructor(environment: string, mathInput: string, modifier: string) {
    ...
    this.axis=new Axis().universal(mathInput, FormatTikzjax,anchorArr,anchor?: string)
    this.addGraph();
  }
  ...
  addGraph() {
    const targetSize = 10;
    const maxComponent = Math.max(Math.abs(this.axis.cartesianX), Math.abs(this.Ycomponent));
